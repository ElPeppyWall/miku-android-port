...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		camHUDE = new FlxCamera();
		camHUDE.bgColor.alpha = 0;

		FlxG.cameras.reset(camGame);
		FlxG.cameras.add(camHUD);
		FlxG.cameras.add(camHUDE);
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'ugh' | 'guns': 
			{
				...
				case 'popipo' | 'aishite' | 'siu' | 'disappearance' | 'chug': 
				{
					defaultCamZoom = 0.8;
					curStage = 'staasge';
					var bg:FlxSprite = new FlxSprite(-500, -200).loadGraphic(Paths.image('concert/back'));
					...
					var stageFront:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('concert/front'));
					...
					tankBop6 = new FlxSprite(-500, 650);
					tankBop6.frames = Paths.getSparrowAtlas('concert/bunch_of_simps');
					tankBop6.animation.addByPrefix('bop','Downer Crowd Bob', 24);
					tankBop6.scrollFactor.set(1.5, 1.5);
					tankBop6.antialiasing = true;
		...
		healthBar.cameras = [camHUDE];
		healthBarBG.cameras = [camHUDE];
		iconP1.cameras = [camHUDE];
		iconP2.cameras = [camHUDE];
	...
	function mikuAnimation(?dialogueBox:DialogueBox, bside:Bool):Void
		{
			...
			{
				case 'popipo':
					trace('funny po pee po!!!');
					inCutscene = true;
					remove(dad);
					var animation:FlxSprite = new FlxSprite(dad.x, dad.y);
					animation.frames = Paths.getSparrowAtlas('concert/mikuintro');
					animation.animation.addByPrefix('startup', 'miku intro instance 1', 24, false);
					...
					var hola:FlxSound = new FlxSound().loadEmbedded(Paths.sound('cheeringppl'), true);
					var BEEP:FlxSound = new FlxSound().loadEmbedded(Paths.sound('beepboop'));
					hola.fadeIn();
					camFollow.setPosition(dad.getMidpoint().x + 40, dad.getMidpoint().y + 20);
										...
										switch(animation.animation.frameIndex)
										{
											case 21:
												...
												if (!BEEP.playing)
													{
														hola.fadeOut();
														camFollow.setPosition(dad.getMidpoint().x + 660, dad.getMidpoint().y - 1);
													}
											case 25:
												trace('BEEP');
												if (!BEEP.playing)
				...
				case 'disappearance':
					trace('desaparicion :V');
					inCutscene = true;
					remove(dad);
					var animation:FlxSprite = new FlxSprite(dad.x, dad.y);
					animation.frames = Paths.getSparrowAtlas('concert/mikumadone');
					animation.animation.addByPrefix('startup', 'miku intro challenge 1', 24, false);
					animation.antialiasing = true;
					add(animation);
					black2.visible = true;
					black3.visible = true;
					...
					var hola:FlxSound = new FlxSound().loadEmbedded(Paths.sound('mikumic'), true);
					var BEEP:FlxSound = new FlxSound().loadEmbedded(Paths.sound('sasadda'));
					camFollow.setPosition(dad.getMidpoint().x + 40, dad.getMidpoint().y - 1);
					FlxG.camera.zoom = 1.3;
	...
	override public function update(elapsed:Float)
	{
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100)
			{
				...
				switch (curStage)
				{
					...
					case 'schoolEvil':
						...
						camFollow.y = boyfriend.getMidpoint().y - 200;
					case 'staasge':
						camFollow.x = boyfriend.getMidpoint().x - 340;
				}
		...
		if (camZooming)
		{
			...
			camHUDE.zoom = FlxMath.lerp(1, camHUDE.zoom, 0.95);
		}
